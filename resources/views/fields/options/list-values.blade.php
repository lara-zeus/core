<div class="space-y-4">
    ...
    <x-zeus::input.group for="dataType-{{ $fld }}" inline label="Data Type">
        <div class="flex justify-between px-2">
            <div class="w-full"><x-zeus::input.radio value="multiple" label="Multiple" id="dataType-{{ $fld }}" wire:model="fields.{{ $sec }}.{{ $fld }}.options.dataType"/></div>
            <div class="w-full"><x-zeus::input.radio value="single" label="Single" id="dataType-{{ $fld }}" wire:model="fields.{{ $sec }}.{{ $fld }}.options.dataType"/></div>
        </div>
    </x-zeus::input.group>
        ...
        <div class="flex justify-between px-2">
            <div class="w-full">
                @if(isset($fields[$sec][$fld]['options']['dataType']) && $fields[$sec][$fld]['options']['dataType'] === 'single')
                    <x-zeus::input.radio value="radio" label="Radio" id="showAs-{{ $fld }}" wire:model="fields.{{ $sec }}.{{ $fld }}.options.showAs"/>
                @else
                    <x-zeus::input.radio value="checkbox" label="Checkboxes" id="showAs-{{ $fld }}" wire:model="fields.{{ $sec }}.{{ $fld }}.options.showAs"/>
                @endif
            </div>
            <div class="w-full"><x-zeus::input.radio value="select" label="Select Menu" id="showAs-{{ $fld }}" wire:model="fields.{{ $sec }}.{{ $fld }}.options.showAs"/></div>
        </div>
    </x-zeus::input.group>
    ...
    <div>
        @if(isset($fields[$sec][$fld]['options']['showAs']) && $fields[$sec][$fld]['options']['showAs'] === 'select')
            <x-zeus::input.group for="searchable-{{ $fld }}" inline label="Allow search">
                <div class="flex justify-between px-2">
                    <div class="w-full"><x-zeus::input.radio value="searchable" label="Yes" id="searchable-{{ $fld }}" wire:model="fields.{{ $sec }}.{{ $fld }}.options.searchable"/></div>
                    <div class="w-full"><x-zeus::input.radio value="not-searchable" label="No" id="searchable-{{ $fld }}" wire:model="fields.{{ $sec }}.{{ $fld }}.options.searchable"/></div>
                </div>
            </x-zeus::input.group>
        @endif
    </div>
