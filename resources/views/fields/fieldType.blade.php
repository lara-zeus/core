<div class="space-y-4">
    <x-zeus::input.group for="field-type-{{ $fld }}" inline label="Field Type" :error="$errors->first('fields.'.$sec . '.'. $fld.'.type')">
        <x-zeus::input.select id="field-type-{{ $fld }}" wire:model="fields.{{ $sec }}.{{ $fld }}.type" x-ref="fieldType" x-on:change="showOptionsFor = $refs.fieldType.value">
            ...
            @foreach(\LaraZeus\Core\Classes\Helpers::availableFields()->groupBy('isZeus')->sort() as $zeus => $inputs)
                @if($loop->count > 1)
                    @if($zeus)
    ...
    @if(!empty($fields[$sec][$fld]['type']) && !in_array($fields[$sec][$fld]['type'],\LaraZeus\Core\Classes\Helpers::availableFields()->where('settings_view', '===', null)->pluck('type')->toArray(),true))
        <div x-cloak>
            ...
            <div x-show.transition.origin.top.left="isCollapsed">
                @foreach(\LaraZeus\Core\Classes\Helpers::availableFields()->where('settings_view', '!=', null) as $fieldOptions)
                    <div x-show="showOptionsFor === '{{ $fieldOptions['type'] }}'">
                        @include('zeus::fields.options.'.$fieldOptions['settings_view'],['fld'=>$fld, 'type'=>$fieldOptions['type']])
                    </div>
                @endforeach
            </div>
        </div>
