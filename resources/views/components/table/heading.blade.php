...
<th {{ $attributes->merge(['class' => 'px-6 py-3 bg-gray-50'])->only('class') }}>
    ...
    @else
        <button {{ $attributes->except('class') }} class="flex items-center space-x-1 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider group focus:outline-none">
            <span>{{ $slot }}</span>
