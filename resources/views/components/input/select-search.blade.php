{{--
Data:
$this->allCollection = json_encode(Collection::orderBy('id', 'desc')->pluck('name', 'id')->toArray(), JSON_THROW_ON_ERROR);
Usage:
<x-zeus::input.select-search :data="$allCollection" placeholder="select data source" wire:model="fields.{{ $sec }}.{{ $fld }}.options.dataSource" id="dataSource-{{ $fld }}" >
    <div>
        @if($fields[$sec][$fld]['options']['dataSource'])
            <x-slot name="addOne">
                <x-zeus::elements.link wire:click="addCollection({{$fld}})" data-tippy-content="Edit this List">
