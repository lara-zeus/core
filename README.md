<p align="center">
<a href="https://larazeus.com"><img src="https://larazeus.com/images/core-banner.png?v=2" /></a>
</p>

<p align="center">

[![Latest Version on Packagist](https://img.shields.io/packagist/v/lara-zeus/core.svg?color=yellow&style=flat-square&label=Zeus-Core&logo=data:image/png;base64,)](https://packagist.org/packages/lara-zeus/core)
[![Latest Version on Packagist](https://img.shields.io/packagist/v/lara-zeus/core.svg?style=flat-square)](https://packagist.org/packages/lara-zeus/core)
[![Tests](https://img.shields.io/github/actions/workflow/status/lara-zeus/core/run-tests.yml?label=tests&style=flat-square&branch=main)](https://github.com/lara-zeus/core/actions?query=workflow%3Arun-tests+branch%3Amain)
[![Code Style](https://img.shields.io/github/actions/workflow/status/lara-zeus/core/fix-php-code-style-issues.yml?label=code-style&flat-square)](https://github.com/lara-zeus/core/actions?query=workflow%3Afix-php-code-style-issues+branch%3Amain)
[![Total Downloads](https://img.shields.io/packagist/dt/lara-zeus/core.svg?style=flat-square)](https://packagist.org/packages/lara-zeus/core)
[![Total Downloads](https://img.shields.io/github/stars/lara-zeus/core?style=flat-square)](https://github.com/lara-zeus/core)

</p>

<h1 align="center">Lara Zeus Core</h1>

thia is the main UI core, and blade components for all lara-zeus packages
>small tasks can be time-consuming, let us build these for you,

## Support Filament

<a href="https://github.com/sponsors/danharrin">
<img width="320" alt="filament-logo" src="https://filamentadmin.com/images/sponsor-banner.jpg">
</a>

## Why this a separate package
we have many packages, some are already published, and more in the way.
so it make sense to separate the assets and the default layouts we're using in our packages.

## Can I customize these views
Yes of course, you can customize the blade files, and make your own themes

#### to publish the views
```bash
php artisan vendor:publish --tag=zeus-views
```
#### to publish the assets files
```bash
php artisan vendor:publish --tag=zeus-assets
```

## More Info
check our packages at [Larazeus](https://larazeus.com/) website
